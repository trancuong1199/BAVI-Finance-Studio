...
import { Layers, Repeat, X, Menu, Bot, Send, Settings, LayoutGrid, RefreshCw, BarChart3, FileText, Code, BookOpen, ChevronDown, ChevronRight, HelpCircle, Zap, Presentation } from 'lucide-react';
...
type ViewState = 'dashboard' | 'swap' | 'uniswap' | 'payments' | 'logs' | 'analytics' | 'faucet' | 'contracts' | 'doc' | 'memos' | 'merchant-treasury' | 'bridge' | 'arbitrage' | 'presentation';
...
`;
  ...
  return (
    <>
      <BackgroundAnimation />
      <div className="mobile-header" style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '10px 15px', justifyContent: 'space-between', alignItems: 'center' }}>
        <div className="nav-brand" onClick={() => navigateTo('dashboard')} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', background: 'none', WebkitTextFillColor: 'initial', color: '#0f172a', margin: 0, padding: 0 }}>
          <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'linear-gradient(135deg, #0ea5e9, #0d9488)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800 }}>B</div>
      ...
      <div className="app-container" style={{ display: 'flex', gap: '1.5rem', background: 'transparent', padding: '1.5rem', minHeight: '100vh', boxSizing: 'border-box' }}>
        <aside className={`sidebar animate-fade-in ${isMobileMenuOpen ? 'open' : ''}`} style={{
          ...
          >
            <div style={{ width: '42px', height: '42px', borderRadius: '12px', background: '#0ea5e9', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: '1.4rem', boxShadow: '0 4px 12px rgba(14, 165, 233, 0.15)', flexShrink: 0 }}>
              B
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
              <div style={{ fontSize: '1.25rem', fontWeight: 800, color: '#0f172a', lineHeight: 1, fontFamily: "'Outfit', 'Inter', sans-serif" }}>BAVI</div>
              <div style={{ fontSize: '0.75rem', color: '#94a3b8', fontWeight: 600, fontFamily: "'Outfit', 'Inter', sans-serif" }}>Finance Studio</div>
            </div>
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', flex: 1, overflowY: 'auto', overflowX: 'hidden' }}>
            {[
              { label: 'Dashboard', view: 'dashboard', icon: <LayoutGrid size={20} /> },
              { label: 'Presentation Deck', view: 'presentation', icon: <Presentation size={20} />, badgeText: 'DECK' },
              { label: 'Arbitrage Bot', view: 'arbitrage', icon: <Zap size={20} />, badge: true },
              ...
              { label: 'Tx Memos', view: 'memos', icon: <FileText size={20} /> },
              { label: 'Custom Contracts', view: 'merchant-treasury', icon: <Code size={20} /> },
              { label: 'Docs', view: 'doc', icon: <BookOpen size={20} /> }
            ].map(item => {
              ...
              return (
                ...
                >
                  ...
                  {item.badge && (
                    <span style={{ background: 'linear-gradient(135deg, #0d9488, #0ea5e9)', color: '#fff', fontSize: '0.65rem', padding: '2px 6px', borderRadius: '6px', fontWeight: 700, fontFamily: "'Outfit', sans-serif" }}>BOT</span>
                  )}
                  {item.badgeText && (
                    <span style={{ background: 'linear-gradient(135deg, #38bdf8, #8b5cf6)', color: '#fff', fontSize: '0.65rem', padding: '2px 6px', borderRadius: '6px', fontWeight: 700, fontFamily: "'Outfit', sans-serif" }}>{item.badgeText}</span>
                  )}
                </a>
              );
            })}
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', textAlign: 'left' }}>
                  <span style={{ fontSize: '0.9rem', fontWeight: 700, color: '#0f172a', lineHeight: 1.1 }}>
                    {address ? `${address.slice(0, 6)}...${address.slice(-4)}` : 'Connect Wallet'}
                  </span>
                  <span style={{ fontSize: '0.725rem', fontWeight: 600, color: address ? '#10b981' : '#64748b', lineHeight: 1.1 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '1.5rem', minWidth: 0 }}>
          <header style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', height: '40px', background: 'transparent', boxSizing: 'border-box', gap: '12px' }}>
            <button onClick={() => connectWallet()} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#fff', border: '1px solid #e2e8f0', padding: '6px 12px', borderRadius: '10px', fontSize: '0.85rem', fontWeight: 600, color: '#0f172a', cursor: 'pointer', outline: 'none', boxShadow: '0 1px 2px rgba(0,0,0,0.02)' }}>
              {connectedWalletInfo ? (
